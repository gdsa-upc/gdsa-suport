...
def display(params,query_id,ranking,relnotrel):
    ...
    # Read query image
    query_im =  cv2.imread(os.path.join(params['root'],params['database'],
                           params['split'], 'images',
                           query_id))
    ...
    query_im = cv2.copyMakeBorder(query_im,100,100,100,100,
    ...
    for i in range(15):
        ...
        im =  cv2.imread(os.path.join(params['root'],params['database'],
                        'train','images',ranking[0].tolist()[i] + '.jpg'))
...
def read_annotation(params):
    ...
    annotation_val = pd.read_csv(os.path.join(params['root'],
...
def AveragePrecision(relist):
    '''
    Takes a hit & miss list with ones and zeros
    and computes its average precision
    '''
    ...
    # When we finish, we divide by the total number of relevant instances,
    # which is the sum of ones in the list
    return (accu/np.sum(relist))

def load_ranking(params,query_id, annotation_val):

    '''
    Loads and  returns the ranking from the txt.
    Returns the true class of the query image as well.
    '''

    # Get the true class of the validation image for which
    # we will evaluate the ranking
    query_class = list(annotation_val.loc[annotation_val['ImageID']
                                == query_id.split('.')[0]]['ClassID'])[0]

    # Open its ranking file
    ranking = pd.read_csv(os.path.join(params['root'],params['root_save'],
                        params['rankings_dir'],params['descriptor_type'],
                        params['split'],query_id.split('.')[0] + '.txt'),
                        header= None)
...
def eval_rankings(params):
    ...
    for val_id in os.listdir(os.path.join(params['root'],params['root_save'],
                    params['rankings_dir'],
...
def single_eval(params,query_id):
    ...
    # We get the true annotations of both sets.
    # We do this in order to display this information as well.
    annotation_val, annotation_train = read_annotation(params)
